...
class DatabaseRepositoryImpl @Inject constructor(): DatabaseRepository {
    ...
    override suspend fun getExercises(workoutId: String): List<Exercise> {
        val workoutDoc = db.collection("workouts").document(workoutId)
        Log.d("db", "workout doc data: ${workoutDoc.get().result.data}")
        return emptyList()
    }
    ...
    override suspend fun getScheduledWorkouts(): List<ScheduledWorkout> {
        TODO("Not yet implemented")
    }
